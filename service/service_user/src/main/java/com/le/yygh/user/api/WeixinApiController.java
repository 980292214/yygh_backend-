...
import com.le.yygh.user.utils.ConstantWxPropertiesUtils;
import com.le.yygh.user.utils.HttpClientUtils;
...
public class WeixinApiController {
    ...
    public String callback(String code,String state) {
        ...
        try {
            ...
            String access_token = jsonObject.getString("access_token");//访问凭证
            String openid = jsonObject.getString("openid");//微信唯一标识
            ...
            if(userInfo == null) { //数据库不存在微信信息
                ...
                JSONObject resultUserInfoJson = JSONObject.parseObject(resultInfo);//字符串转为JSONObject
                //解析用户信息
                //用户昵称
                String nickname = resultUserInfoJson.getString("nickname");
                //用户头像
                String headimgurl = resultUserInfoJson.getString("headimgurl");//没用上
            ...
            String name = userInfo.getName();//真名
            ...
            if(StringUtils.isEmpty(userInfo.getPhone())) {//绑定手机
                map.put("openid", userInfo.getOpenid());
            } else {//有手机号了,不用绑定,但要判断用户有没有被锁定(禁用)
                map.put("openid", "");
                //校验是否被禁用，自己加5.3--
    ...
    public Result genQrConnect() {
        try {
            ...
            wxOpenRedirectUrl = URLEncoder.encode(wxOpenRedirectUrl, "utf-8");//要使用urlEncode对链接进行处理
