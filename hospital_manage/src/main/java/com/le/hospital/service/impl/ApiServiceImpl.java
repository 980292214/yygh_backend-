...
import com.le.hospital.mapper.HospitalSetMapper;
import com.le.hospital.model.HospitalSet;
...
import com.le.hospital.util.MD5;
import com.le.hospital.mapper.ScheduleMapper;
import com.le.hospital.util.BeanUtils;
...
public class ApiServiceImpl implements ApiService {
    ...
    public String getHoscode() {
        HospitalSet hospitalSet = hospitalSetMapper.selectById(1);//写死？ TODO 5.10
    ...
    private String getApiUrl() {//没有重写 5.10
        HospitalSet hospitalSet = hospitalSetMapper.selectById(1);
    ...
    @Override
    //上传（保存）医院的基本信息与规则信息，发送请求给 yygh_hosp 系统
    public boolean saveHospital(String data) {
        ...
        paramMap.put("sign", MD5.encrypt(this.getSignKey()));//todo

        JSONObject respone =
                HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/api/hosp/saveHospital");//8201端口
        System.out.println(respone.toJSONString());
    ...
    public boolean saveSchedule(String data) {
        ...
        for(int i=0, len=jsonArray.size(); i<len; i++) {
            ...
            System.out.println("hosp微服务的respone:"+respone.toJSONString());
            if(null == respone || 200 != respone.getIntValue("code")) {
                throw new YyghException(respone.getString("message"), 201);
            }
        }
